...
def main(args):
  ...
  for y in range(datai, dataf+1):
    ...
    for m in range(1, 13):
      ...
      if os.path.exists(f"{store}/t2m_p4PGW_m4CTRL_{y}_{m:02d}.p"):
        print("jump")
        continue      
      ...
      # CTRL -12C bin; Boolean array with the places where the temperature is between -16 and -12 in CTRL
      count_bool_le = np.less(t2.values, 257.15)
      count_bool_ge = np.greater_equal(t2.values, 261.15)
      between = count_bool_ge + count_bool_le
      ctrl_minus12 = np.invert(between)
      ...
      # CTRL +4C bin; Boolean array with the places where the temperature is between +2 and +6 in CTRL
      count_bool_le = np.less(t2.values, 275.15)
      count_bool_ge = np.greater_equal(t2.values, 279.15)
      between = count_bool_ge + count_bool_le
      ctrl_plus4 = np.invert(between)    
      ...
      # PGW -4C bin; Boolean array with the places where the temperature is between -6 and -2 in PGW
      count_bool_le = np.less(t2_pgw.values, 267.15)
      count_bool_ge = np.greater_equal(t2_pgw.values, 271.15)
      between = count_bool_ge + count_bool_le
      pgw_minus4 = np.invert(between)
        
      # PGW +4C bin; Boolean array with the places where the temperature is between +2 and +6 in PGW
      count_bool_le = np.less(t2_pgw.values, 275.15)
      count_bool_ge = np.greater_equal(t2_pgw.values, 279.15)
      between = count_bool_ge + count_bool_le
      pgw_plus4 = np.invert(between)    
      ...
      count = np.count_nonzero(pgw_minus4.astype(int), axis=0)
      print(count.shape)
      pickle.dump( count, open( f"{store}/t2m_minus4PGW_{y}_{m:02d}.p", "wb" ) )
    
      count = np.count_nonzero(pgw_plus4.astype(int), axis=0)           
      print(count.shape)
      pickle.dump( count, open( f"{store}/t2m_plus4PGW_{y}_{m:02d}.p", "wb" ) )
      ...
      count = np.count_nonzero(ctrl_plus4.astype(int), axis=0)
      print(count.shape)
      pickle.dump( count, open( f"{store}/t2m_plus4CTRL_{y}_{m:02d}.p", "wb" ) )
     
      count = np.count_nonzero(ctrl_minus4.astype(int), axis=0)  
      print(count.shape)
      pickle.dump( count, open( f"{store}/t2m_minus4CTRL_{y}_{m:02d}.p", "wb" ) )
        
      #count = np.count_nonzero(ctrl_minus8.astype(int), axis=0)           
      #pickle.dump( count, open( f"{store}/t2m_minus8CTRL_{y}_{m:02d}.p", "wb" ) )
        
      #count = np.count_nonzero(ctrl_minus12.astype(int), axis=0)           
      #pickle.dump( count, open( f"{store}/t2m_minus12CTRL_{y}_{m:02d}.p", "wb" ) )
      ...
      # Cases comparing with -4C in PGW
    
      m4PGW_m4CTRL = np.zeros(ctrl_near0.shape)
      m4PGW_m4CTRL[(pgw_minus4 == True) & (ctrl_minus4 == True)] = 1
      print(m4PGW_m4CTRL.shape)
      print(ctrl_minus4.shape)
      print(pgw_minus4.shape)
      count = np.count_nonzero(m4PGW_m4CTRL.astype(int), axis=0)
      print(count.shape)
      pickle.dump( count, open( f"{store}/t2m_m4PGW_m4CTRL_{y}_{m:02d}.p", "wb" ) )
    
      m4PGW_m8CTRL = np.zeros(ctrl_near0.shape)
      m4PGW_m8CTRL[(pgw_minus4 == True) & (ctrl_minus8 == True)] = 1
      print(m4PGW_m8CTRL.shape)
      print(ctrl_minus8.shape)
      print(pgw_minus4.shape)
      count = np.count_nonzero(m4PGW_m8CTRL.astype(int), axis=0)           
      print(count.shape)
      pickle.dump( count, open( f"{store}/t2m_m4PGW_m8CTRL_{y}_{m:02d}.p", "wb" ) )
    
      m4PGW_m12CTRL = np.zeros(ctrl_near0.shape)
      m4PGW_m12CTRL[(pgw_minus4 == True) & (ctrl_minus12 == True)] = 1
      count = np.count_nonzero(m4PGW_m12CTRL.astype(int), axis=0)           
      pickle.dump( count, open( f"{store}/t2m_m4PGW_m12CTRL_{y}_{m:02d}.p", "wb" ) )
    
      # Cases comparing with +4 in PGW
    
      p4PGW_p4CTRL = np.zeros(ctrl_near0.shape)
      p4PGW_p4CTRL[(pgw_plus4 == True) & (ctrl_plus4 == True)] = 1
      count = np.count_nonzero(p4PGW_p4CTRL.astype(int), axis=0)           
      pickle.dump( count, open( f"{store}/t2m_p4PGW_p4CTRL_{y}_{m:02d}.p", "wb" ) )
    
      p4PGW_near0CTRL = np.zeros(ctrl_near0.shape)
      p4PGW_near0CTRL[(pgw_plus4 == True) & (ctrl_near0 == True)] = 1  
      count = np.count_nonzero(p4PGW_near0CTRL.astype(int), axis=0)           
      pickle.dump( count, open( f"{store}/t2m_p4PGW_n0CTRL_{y}_{m:02d}.p", "wb" ) )
    
      p4PGW_m4CTRL = np.zeros(ctrl_near0.shape)
      p4PGW_m4CTRL[(pgw_plus4 == True) & (ctrl_minus4 == True)] = 1
      count = np.count_nonzero(p4PGW_m4CTRL.astype(int), axis=0)           
      pickle.dump( count, open( f"{store}/t2m_p4PGW_m4CTRL_{y}_{m:02d}.p", "wb" ) )                          
