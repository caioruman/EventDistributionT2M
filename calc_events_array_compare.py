...
#        continue
      ...
      if os.path.exists(f"{store}/t2m_compare_pr_{y}_{m:02d}.p"):
        print("jump")
        continue      
      ...
      count_bool_le = np.less_equal(t2.values, 271.15)
      count_bool_ge = np.greater_equal(t2.values, 275.15)      
      between = count_bool_ge + count_bool_le
      aux = np.invert(between)
    
      count_bool_le = np.less_equal(t2_pgw.values, 271.15)
      count_bool_ge = np.greater_equal(t2_pgw.values, 275.15)      
      between = count_bool_ge + count_bool_le
      aux_pgw = np.invert(between)
      
      print(t2.shape, t2_pgw.shape)

      near0_both = np.zeros(aux.shape)
      near0_both[(aux == True) & (aux_pgw == True)] = 1
      # Counting all the 1 values in the time dimension
      count = np.count_nonzero(near0_both.astype(int), axis=0)      
           
      pickle.dump( count, open( f"{store}/t2m_compare_{y}_{m:02d}.p", "wb" ) )                  
